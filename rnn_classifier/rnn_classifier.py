	...
	def train(self, dataset):
		...
		with tf.Session() as sess:
			...
			while step * batch_size < training_iters:
				...
				# Run optimization op
				sess.run(self.optimizer, feed_dict={self.x: batch_x, self.y: batch_y, self.dropout: dropout, self.sequence_lengths: seq_lengths})
				if step % display_step == 0:
					# Calculate dev accuracy
					dev_x, dev_y = dataset.dev.all()
					seq_lengths = BiRNNClassifier.get_sequence_lengths(dev_x)

					acc = sess.run(self.accuracy, feed_dict={self.x: dev_x, self.y: dev_y, self.dropout: 1.0, self.sequence_lengths: seq_lengths})
					#  Calculate batch loss
					loss = sess.run(self.cost, feed_dict={self.x: dev_x, self.y: dev_y, self.dropout: 1.0, self.sequence_lengths: seq_lengths})
