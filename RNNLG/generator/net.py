...
class Model(object):
	...
	def trainNetDT(self):
		...
		while True:
			...
			sec = (time.time() - tic) / 60.0
			if self.debug:
				print 'Epoch %2d, Alpha %.4f, TRAIN Obj:%.4f, Expected BLEU:%.4f, Expected ERR:%.4f, Time:%.2f mins,' % (
					epoch, self.lr, train_obj / float(num_sent), train_bleu / float(num_sent),
					train_err / float(num_sent),
					sec),
				sys.stdout.flush()
	...
	def eval(self):
		...
		while True:
			...
			dact = raw_input('Enter dact: ')

			# read data point
			data = self.reader.processSingleDact(dact)
			if data is None:
				print 'NONE'
			a, sv, s, v = data
			# remove batch dimension
			...
			gens = self.model.gen(a, sv, s, v)
			# for slot error rate scoring
			felements = [self.reader.cardinality[x + self.reader.dfs[1]] for x in sv]
			...
			for penalty, gen in gens:
				# score slot error rate
				cnt, total, caty = self.gentscorer.scoreERR(a, felements, self.reader.delexicalise(gen, dact))
				# accumulate slot error cnts
				gencnts[0] += cnt
				gencnts[1] += total
				gencnts[2] += caty
